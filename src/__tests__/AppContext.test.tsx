...
function ContextExtractor(props: {
  children: (context: ContextValues) => JSX.Element;
}) {
  ...
  const { rerender } = render(
    <MemoryRouter initialEntries={["/?type=example&location=testing"]}>
      <AppProvider>
        <ContextExtractor>
          {(context) => (
            <span data-testid="value">Received: {context.type}</span>
          )}
        </ContextExtractor>
      </AppProvider>
    </MemoryRouter>
  ...
  rerender(
    <MemoryRouter initialEntries={["/?type=example&location=testing"]}>
      <AppProvider>
        <ContextExtractor>
          {(context) => (
            <span data-testid="value">Received: {context.location}</span>
          )}
        </ContextExtractor>
      </AppProvider>
    </MemoryRouter>
          ...
          {(context) => (
            <>
              <span data-testid="selected">
                Selected: {context.piercings[0].selected ? "true" : "false"}
              </span>
              <button
                role="button"
                onClick={() =>
                  context.handleBtns(
                    context.piercings[0].nodeid,
                    context.piercings[0].site_cat
                  )
                }
              />
            </>
          ...
          {(context) => (
            <>
              <span data-testid="filter">
                Selected: {context.modFilters.join(",")}
              </span>
              <button
                role="button"
                onClick={() => context.handleModFilterChange("isp_gold")}
              />
            </>
          )}
        </ContextExtractor>
        ...
        <ContextExtractor>
          {(context) => (
            <>
              ...
              <button
                role="button"
                onClick={() => context.handleFilterChange("type", now)}
              />
            </>
          )}
        </ContextExtractor>
          ...
          {(context) => (
            <>
              <span data-testid="type">
                Selected: {[context.type, context.location]}
              </span>
              <button
                role="button"
                onClick={() => context.handleClearFilters()}
              />
            </>
          )}
        </ContextExtractor>
          ...
          {(context) => (
            <>
              <span data-testid="session">
                {context.sessionOver ? "SESSION_OVER" : "SESSION_OK"}
              </span>
          ...
          {(context) => (
            <>
              ...
              <button
                id="back-btn"
                role="button"
                onClick={(e) => context.toggleSessionOver(e)}
              />
            </>
          )}
        </ContextExtractor>
